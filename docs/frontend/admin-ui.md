...
- **Message queue** (`app/admin/sequences/[id]/messages/message-queue-client.tsx`): per-row approve / approve_at / reschedule / cancel / reject (with reason) / retry / edit. Bulk equivalents via the action bar. 12s react-query polling; counts derived in-memory via `useMemo` from the cached list. Approval-mode banner renders when `sequence.send_mode === "approval"`. In-house `<ToastViewport>` lives in `components/ui/toast.tsx` (no third-party toast lib).
  - **Status tabs:** All / Pending (`draft`) / **Queued** (`scheduled,sending`) / Sent (`sent,delivered,opened,clicked,replied`) / Failed (`failed,bounced`) / **Rejected** (`rejected`). "Queued" replaced the old "Approved" tab because `scheduled` is written by approvals, auto-mode, retries, and deferrals alike — it means "queued to send," not "a human approved it." Including `sending` here keeps in-flight rows visible instead of falling through every filter. **Rejected** is a dedicated terminal status (migration `036`), kept separate from delivery failures; the row shows a muted rejection banner with the reason rather than the red failure banner.
  - **Stats sidebar:** Total / Pending / Queued / Sending / Sent / Opened% / Replied% / Bounced / Failed / Rejected. "Sent" counts every row that reached at least `sent` (sent+delivered+opened+clicked+replied) so the number matches the Sent tab's row count.
  - **Approve preserves drip cadence:** because generation now pre-creates a draft for every step with its planned `scheduled_at`, the row's "Scheduled" column shows when each step is due, and approving a later step keeps that future time instead of sending immediately.
