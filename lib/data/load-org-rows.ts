...
import type { OrgRow, FilterOptions } from "@/app/admin/organizations/organizations-table-client";

export type LoadOrgRowsResult = {
  rows: OrgRow[];
...
export async function loadOrgRows(supabase: SupabaseClient): Promise<LoadOrgRowsResult> {
  ...
  const [
    ...
    supabase.from("organizations").select("category").not("category", "is", null),
    supabase.from("organization_signals").select("signal_type").not("signal_type", "is", null),
  ]);
  ...
  return { rows, filterOptions, orgPeopleMap };
}
